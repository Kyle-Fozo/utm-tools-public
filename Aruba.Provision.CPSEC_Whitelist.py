...
# ***Ensure that you set your own CONFIG PARAMETERS below.
#
...
# Used to print AP_GROUPs in a more eligant manner.
import pprint
...
#######################
###CONFIG PARAMETERS###
svc_account = "<SERVICE ACCOUNT_NAME>"
mm_vrrp_hostname = "<FQDN>"
md_vrrp_hostname = "<FQDN>"
###CONFIG PARAMETERS###
#######################
...
run = typer.Typer(  # If run as main create a 'typer.Typer' app instance to run the program within.
    ...
    help="Welcome to the Aruba CPSEC Whitelist Provisioning Tool for UTM.\n\nThe Aruba wireless system at UTM utilizes the 'Control Plane Security' (CPSEC) 'Whitelist Database' (WDB) to authorize and provision WAPs onto the Aruba Wireless Platform.  This is done by generating certificates for each WAP we will bring online within the CPSEC WDB.\n\nThis tool has been designed to automate single or batch provisioning of WAPs into the CPSEC WDB, as well as modifying their certs to 'factory-approved' so the registrations do not expire.\n\nYou will require a valid, configured, AP_GROUP along with a MAC_ADDRESS and AP_NAME to be provisioned.\n\nThe 'get-ap-groups' command will assist you in finding valid AP_GROUPs.\n\nThe 'provision' command has both a 'from-cli' and 'from-file' subcommands, as well as detailed HELP sections for every command and subcommand.\n\nRun commands without arguemnts for HELP information, or add the --help option after any command for more information and format requirements.",
...
def Get_AP_Groups(  # Create the 'get-ap-groups' command within typer.
    ...
    try:
        passwd = shell(f"pass show {svc_account}")
        with (
            ArubaRESTAPIClient(
                f"{md_vrrp_hostname}:4343", f"{svc_account}", passwd
            ) as host1
        ):
            ...
            pprint.pprint(controller_ap_groups)
            sys.exit()
...
def From_File(
    ...
    Empty lines are not required in CSV file.
    """
    try:
...
def From_CLI(
    ...
    ***AP_GROUP -MUST- be escaped if group name contains spaces.***\n
    See example above.
    """
    try:
...
def Return_AP_Groups():
    "Gets a valid list of AP_GROUPs from the Managed Device, returns a list, and exists."
    passwd = shell(f"pass show {svc_account}")
    with (
        ArubaRESTAPIClient(
            f"{md_vrrp_hostname}:4343", f"{svc_account}", passwd
...
def Verify_And_Create(input_table: InputTable):
    ...
    with (
        ArubaRESTAPIClient(
            f"{mm_vrrp_hostname}:4343", f"{svc_account}", passwd
        ) as host2
    ):
